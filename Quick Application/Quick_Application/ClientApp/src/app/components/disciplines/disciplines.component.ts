...
export class DisciplinesComponent implements OnInit, OnDestroy  {
  public static readonly DBKeyDisciplines = 'disciplines.todo_list';
  ...
  getFromDisk() {
    return this.localStorage.getDataObject(`${DisciplinesComponent.DBKeyDisciplines}:${this.currentUserId}`);
  }

  saveToDisk() {
    if (this.isDataLoaded) {
      this.localStorage.saveSyncedSessionData(this.rowsCache, `${DisciplinesComponent.DBKeyDisciplines}:${this.currentUserId}`);
    }
  }
}
